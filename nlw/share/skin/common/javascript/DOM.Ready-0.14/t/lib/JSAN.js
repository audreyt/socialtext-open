...
=head3 JSAN.loadType(type)

Specify what loader JSAN should use.  Valid types are "ajax" or
"script".  By default, JSAN will use the "Ajax" loader, which creates
a new XMLHttpRequest for each new script that is loaded.

See L<Loaders> for more details.
...
JSAN.loadType = function (type) {
    if (type.match(/^ajax$/, 'i')) {
        JSAN._loadStrategy = JSAN._loadStrategyAjax;
    } else if (type.match(/^script$/, 'i')) {
        JSAN._loadStrategy = JSAN._loadStrategyScriptInject;
    } else {
        throw new Error("Invalid loadType: " + type + ".  Valid types are 'ajax' or 'script'");
    }
}

JSAN._loadStrategyAjax = function (url) {
    return JSAN._loadJSFromUrl(url);
}

JSAN._loadStrategyScriptInject = function (url) {
    var script = document.createElement("script");
    script.setAttribute('src', url);
    script.setAttribute('type', 'text/javascript');
    script.setAttribute('language', 'Javascript');
    script.setAttribute('createdBy', 'JSAN');
...
JSAN._makeNamespace = function(js, pkg) {
    ...
    eval(js);
    var classdef;
    try { classdef = eval(pkg) }
    catch (e) { /* with script injection the package isn't yet loaded */ }
    for (var i = 0; i < spaces.length; i++) {
        var name = spaces[i];
        if (i == spaces.length - 1) {
            if (typeof parent[name] == 'undefined') {
                parent[name] = classdef;
                // XXX - classdef may not be an object if script
                // injection is used because we may not have loaded
                // the library yet - not sure why this prototype
                // setting is needed anyway - DR
                if ( typeof classdef == 'object' && typeof classdef['prototype'] != 'undefined' ) {
...
JSAN offers several different loaders for loading Javascript libraries
on the fly.  Each of these has different pros and cons.

=over 4

=item * Ajax

This loader use the XMLHttpRequest API to asynchronously load
Javascript.  This loader is the most reliable, and is capable of
trying several different repository paths.  It can be used from any
part of an HTML page, and should work on all the major browsers
(Mozilla, IE, Opera, and Safari at a minimum).

It has several disadvantages.  First, it I<always> reloads the
libraries for each request, as the various XMLHttpRequest
implementations in each browser never send "If-Modified-Since"
headers.  Second, it may obscure compilation errors in the loaded
libraries, because these errors may be reported as coming from the
JSAN library itself.
...
This method uses the DOM to inject <script> elements into the
document's head.  This loader offers several advantages.  First, it
uses the browser's normal loading system, so "If-Modified-Since"
headers are sent is appropriate.  Second, it does not C<eval> the
loaded Javascript file, so errors are reported normally.

It has several downsides, however.  First, if you use this loader
loading will not work once the entire document C<< <head> >> has been
parsed.  In addition, it ignores JSAN's C<errorLevel>.  Finally, JSAN
will normally try to create a "namespace" for each library it loads,
but with script injection this cannot be done.  This means that the
libraries you load must define their own namespace properly.  See
