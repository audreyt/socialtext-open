package Socialtext::File::Copy::Recursive;
# @COPYRIGHT@

# XXX - this is a fixed version of File::Copy::Recursive I'm sticking in the Socialtext source
# tree until the author release the next version. The changes are in
# the dircopy() function. - Dave

# XXX2 - I'm starting to think the maintainer will not be applying my
# patches. Plus I'm not sure if my patches are sufficiently
# generic. I'm somewhat tempted to replace this with something that
# just does the little bit that we need.
...
sub dircopy {
   ...
   $recurs =  sub {
      ...
      for my $file (@files) {
          ...
          if(-l $org) {
              my $targ = readlink($org);
              if (defined($targ)) {
                  $targ = File::Spec->catfile($str, $targ)
                      unless File::Spec->file_name_is_absolute($targ);
              }

              if ($CopyLink || !defined $targ || !-e $targ) {
